...
# Set up basic logging (use Config.LOG_FILE so file exists before config selection)
logging.basicConfig(
    level=logging.DEBUG,
...
CANDIDATE_ENV = (os.environ.get('APP_ENV') or os.environ.get('FLASK_ENV') or 'development').lower()
if CANDIDATE_ENV == 'production':
...
# adjust logger level according to selected config
logger.setLevel(logging.DEBUG if app.config.get('DEBUG') else logging.INFO)
...
if __name__ == '__main__':
    ...
    # When running directly, use the config's host/port and debug flag.
